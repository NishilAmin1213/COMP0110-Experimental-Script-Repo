...
# pip installed mistralai, tree-sitter-java==0.23.2
# pip installed codebleu==0.7.1(via github link)
# pip install mistralai transformers torch hf_xet
from utils import *
...
if __name__ == '__main__':
    # Requires the Mistral API Key to be stored under the environment variable 'MISTRAL_API_KEY'
    print("Program Started")

    # Run the secondary dataset through the pipeline
    print("Started Processing the Secondary Dataset")
    dataset = read_dataset('secondary_dataset.pkl', silent=False)
    run_program(dataset, prompt_mistral_api, "secondary_mistral_results.pkl")
    ...
    #dataset_same = read_dataset('same_param_functions_dataset.pkl', silent=False)
    #dataset_diff = read_dataset('different_param_functions_dataset.pkl', silent=False)
    #run_program(dataset_same, prompt_mistral_api, "mistral_results_same.pkl")
    #run_program(dataset_diff, prompt_mistral_api, "mistral_results_diff.pkl")
